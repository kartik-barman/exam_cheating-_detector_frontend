...
export function LiveMonitoring({ isMonitoring, results }: LiveMonitoringProps) {
  ...
  return (
    ...
    >
      ...
      {hasPermission === false ? (
        <div className="w-full h-full flex flex-col items-center justify-center gap-4 text-red-500 bg-red-500/5">
          <Camera className="w-16 h-16 opacity-40" />
          <p className="font-medium">Camera permission denied</p>
          <p className="text-xs opacity-60">Please enable camera access in your browser</p>
        </div>
      ) : (
        <video 
          ref={videoRef}
